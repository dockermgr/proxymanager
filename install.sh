...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
##@Version           :  202410121636-git
# @@Author           :  Jason Hempstead
# @@Contact          :  jason@casjaysdev.pro
# @@License          :  LICENSE.md
# @@ReadME           :  install.sh --help
# @@Copyright        :  Copyright: (c) 2024 Jason Hempstead, Casjays Developments
# @@Created          :  Saturday, Oct 12, 2024 16:36 EDT
# @@File             :  install.sh
# @@Description      :  Container installer script for proxymanager
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
export APPNAME="proxymanager"
export VERSION="202410121636-git"
...
setopts=$(getopt -o "a:,i,s:,h:,d:,e:,m:,p:" --long "name:,init,server:,host:,domain:,env:,mount:,port:" -n "$APPNAME" -- "$@" 2>/dev/null)
...
DOCKER_REGISTRY_REPO_NAME="proxy-manager"
...
run_post_custom() {
  ...
  __printf_spacing_color 128 "$DOCKERMGR_INSTALL_SCRIPT and set --publish 81:81 to --publish 127.0.0.1:81:81"
...
CONTAINER_WEB_SERVER_INT_PORT="81"
...
CONTAINER_ADD_CUSTOM_PORT="80:80/tcp,443:443/tcp,81:81/tcp"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
CONTAINER_MOUNT_CONFIG_MOUNT_DIR="/data/nginx"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Define additional mounts - add HOST/ to use $DATA_DIR/rootfs - [/dir:/dir,/otherdir:/otherdir]
CONTAINER_MOUNTS="/etc/letsencrypt:/etc/letsencrypt"
CONTAINER_MOUNTS+=""
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
CONTAINER_DEFAULT_USERNAME="admin@example.com"
CONTAINER_DEFAULT_PASSWORD="changeme"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
DB_SQLITE_FILE="\${DB_SQLITE_FILE:-$DATABASE_DIR_SQLITE/nginxproxy.db}"
EOF
}
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Set custom script - saves to /config/env/proxymanager.script.sh
...
if [ -n "$PROXYMANAGER_USERNAME" ]; then
  CONTAINER_USER_NAME="$PROXYMANAGER_USERNAME"
fi
if [ -n "$CONTAINER_USER_NAME" ]; then
  CONTAINER_USER_NAME="${PROXYMANAGER_USERNAME:-${CONTAINER_USER_NAME:-$DEFAULT_USERNAME}}"
fi
if [ -n "$CONTAINER_USER_NAME" ]; then
...
if [ -n "$PROXYMANAGER_PASSWORD" ]; then
  CONTAINER_USER_PASS="$PROXYMANAGER_PASSWORD"
fi
if [ -n "$CONTAINER_USER_PASS" ]; then
  ...
  CONTAINER_USER_PASS="${PROXYMANAGER_PASSWORD:-${CONTAINER_USER_PASS:-$DEFAULT_PASSWORD}}"
fi
if [ -n "$CONTAINER_USER_PASS" ]; then
